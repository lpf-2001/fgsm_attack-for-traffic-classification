...
def FillImage(step,classname,count,imagepath):
    ...
    for z in range(1,quot+1):
        ...
        temp += 1
    return temp
def ReadFile(path,classification,count,imagepath,OversamplingClassname):
    '''
            ...
            if onepcap.startswith(classname) and onepcap.endswith(".pcap"):
                with open(path + "\\" + onepcap, 'rb') as f:
                    ...
                    while (i < len(data)):
                        ...
                        i = i + packet_len + 16
                        #if classname in OversamplingClassname and step == 2000:
                        if classname == "Gmail" and step == 800:
                            break
                        if classname == "Outlook" and step == 500:
                            break
                        if classname == "Facetime" and step == 300:
                            break
                        if step >= count:
                            break
                print(onepcap,step)
...
if __name__ == '__main__':
    path = "E:\\DL\\DataSet\\USTC-TFC2016"
    imagepath = "E:\\DLResult\\simple\\USTC-TFC2016simple\\TransformImage"
    classification = ['BitTorrent','Facetime','FTP','Gmail','MySQL','Outlook','Skype','SMB','Weibo','WorldOfWarcraft']
    OversamplingClassname = ['Gmail','Outlook','MySQL']
    ReadFile(path,classification,2000,imagepath,OversamplingClassname)
